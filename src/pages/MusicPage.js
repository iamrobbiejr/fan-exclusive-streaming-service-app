...
import ArtistMusicCard from "../components/ArtistMusicCard";
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <section className="mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {artists.slice(0, 4).map(artist => (
            <ArtistMusicCard key={artist.id} artist={artist} />
          ))}
        </div>
      </section>
      ...
      <section className="mb-12">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link to="/albums" className="text-amber-600 hover:underline">View All Albums</Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      ...
      <section>
        ...
        <div className="bg-amber-200 rounded-lg shadow-2xl overflow-hidden">
          ...
          {Array.from({ length: Math.ceil(songs.length / songsPerPage) }, (_, i) => (
            <button
              ...
              className={`mx-1 px-3 py-1 rounded ${
                currentPage === i + 1 ? 'bg-amber-600 text-white' : 'bg-gray-200'
              }`}
            >
              {i + 1}
            </button>
